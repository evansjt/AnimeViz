...
SELECT count AS "# BL Anime titles", rating AS "Rating", rank_within_rating AS "Rank", title AS "Title"
FROM (SELECT r.priority, bl.rating, bl.count, a.mal_id, a.title, RANK() OVER (PARTITION BY bl.rating ORDER BY a.popularity ASC) rank_within_rating
FROM anime a
LEFT JOIN genre g ON a.mal_id = g.mal_id
