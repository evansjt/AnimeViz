...
function readSqlDataToOutput(rows, dataOutput) {
    const animeTitles = rows.map(r => ({ title: r.Title, url: r.url, status: r.status, rank: `#${r.rank}`, episodesAired: r['# Episodes'], airespan: { daysAired: r['Time Aired'], airedFrom: new Date(r['Aired From']).toLocaleDateString(), airedTo: new Date(r['Aired To']).toLocaleDateString() } }));
